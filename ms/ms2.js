...
-----------------------------------------------------------
🤩.关于重定向？
如果服务器端不返回301或302(这种状态码浏览器会自己进行跳转，前端不能控制)，可以返回一个用于重定向的url，然后前端从响应中读取到这个url，
可以使用window.location.href进行跳转，如果在vue或react的框架中，可以使用各自的路由进行跳转。
------------------------------------------------------------
🤩.HTTP(响应)状态码？(🌸)
  1. 1XX:
  ...
  3. 3xx:
         301 Moved Permanently 永久重定向，当网页或资源的URL永久改变时使用，使用301重定向时，搜索引擎会更新其索引来反映新的url，并将旧url的大部分链接权重传递给新的url(seo友好)
         浏览器会缓存301重定向，在第一次重定向之后，如果再次访问，会直接跳转到重定向的地址
         302 Found 暂时重定向 ，搜索引擎不会更新索引来反映新的url，且浏览器不会缓存302，所以用户每次访问旧url时，浏览器都会向服务器发送请求。 
         304 Not Modified  告诉客户端不用发送请求，可以使用当前缓存。当客户端通过发送一个带有If-Modified-Since或If-None-Match头部的HTTP请求来检查资源是否更新时，
         如果资源自从上次请求后就没有变化，服务器就会返回304状态码(304状态码仅适用于GET和HEAD请求)
  4. 4xx:
         ...
         (在访问1.受保护的资源  2.身份验证机制(如JWT认证失败时) 3.会话过期(用户已经登录但会话过期的情况下))
...
->强缓存过期后，浏览器会向服务器发送请求来检查资源是否更新，协商缓存通过Last-Modified/If-Modified-Since 和ETAG/If-None-Match头来实现  
 协商缓存:
 1.Last-modified：服务器先读取出文件的修改时间，然后将它赋值给响应头字段last-Modified，浏览器再次请求资源时，会发送If-Modified-Since头，
   包含它上次收到的Last-Modified值，然后在这之后的每次请求，都会在响应头中携带此字段，然后服务端接收到请求后会和自己现在的修改时间作比较来决定使用缓存还是返回新资源。
 2。ETag：服务器读出资源的hash值(文件指纹)，然后赋值给ETag响应头字段，下次浏览器在请求时，会自动读取出响应头中的ETag，并把它的值赋给if-none-Match请求头，
   服务端在接收到这个字段后，会把它的值和当前hash值比较，如果两个hash值完全一致，说明文件没有被修改，可以使用缓存(于是返回304状态码，返回空的响应体，
   命中协商缓存)；如果不一致，会发送一个新的ETag值。
