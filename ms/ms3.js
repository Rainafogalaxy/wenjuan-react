...
62：对Promise的理解？
——————————————————————————————————————————————————————————————————————————————————————
63：事件冒泡是啥，在js和vue中分别怎么取消？
    事件冒泡就是，比如一个元素外边套了一个元素然后我在里边的元素中添加了一个点击事件，来触发对应的回调函数(实际上会套很多层，冒泡行为会直到文档的根元素为止)，
...
——————————————————————————————————————————————————————————————————————————————————————————————————
64：loader与plugin的区别？
65：如果想要在打包的时候将项目中的px单位都改为rem如何做？
66：输入url到页面渲染的过程？(...hh)
67: git有哪些命令？以及版本控制？
————————————————————————————————————————————————————————————————————————————————————————————————————
68：盒子模型？
   CSS有两种盒子模型：
      通过box-sizing：content-box；
                      border-box；来设置，其中content-box属性值表示当前元素的宽高计算没有内外边距和边框，
                      而border-box会包含内外边距和边框的宽度，用来计算整个盒子的大小；
—————————————————————————————————————————————————————————————————————————————————————————————————————
69：TS中type与interface的区别？any和泛型的区别？
    -->type与interface：
       -->type和interface都可以描述一种类型，语法上的区别是type使用交集 '&'实现继承，且定义的类型可以是任何类型，包括基本数据类型和复杂数据类型；
    ...
    -->any与泛型：
       ...
       -->泛型:使用了泛型的类型，可以让开发者在编写代码时不直接定义所用到的变量或对象的类型，而是通过参数传递来的数据类型来判断，最终确定数据类型
——————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
70：TS比起JS的区别和优点？
